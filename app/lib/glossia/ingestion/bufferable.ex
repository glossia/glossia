defmodule Glossia.Ingestion.Bufferable do
  @moduledoc """
  ...
  defmacro __using__(_opts) do
    quote do
      @before_compile Glossia.Ingestion.Bufferable
    end
  end
  ...
  defmacro __before_compile__(_env) do
    quote do
      ...
      """
      def buffer_opts do
        Glossia.Ingestion.Bufferable.compile_time_prepare(__MODULE__)
      end
    end
  end

  @doc """
  Prepares buffer parameters for a given schema at compile time.

  The `RowBinaryWithNamesAndTypes` format expects a header row with the column names
  that we can generate before runtime, as well as the actual SQL query to insert the data.
  """
