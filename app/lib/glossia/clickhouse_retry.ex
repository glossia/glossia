defmodule Glossia.ClickHouseRetry do
  @moduledoc """
  ...
  Idle pool sockets can be reaped by intermediate NAT and surface on the
  next checkout as `Mint.TransportError{reason: :closed}`; in-flight
  queries on a half-dead socket surface as `:timeout`. Both clear within
  milliseconds and are safe to retry on idempotent reads/writes.

  Used by `Glossia.IngestRepo` (writes: `insert`, `insert_all`) so every
  wired Repo call inherits the retry without per-call-site wrapping.
  """
