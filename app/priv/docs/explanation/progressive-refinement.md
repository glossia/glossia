...
1. **Draft**: Glossia generates a structurally valid first pass based on your source files and the context in `L10N.md`.
2. **Review**: Your team flags issues through pull requests and diffs, the same workflow you already use for code.
3. **Refine**: Updated context files, terminology corrections, and review feedback feed into the next run.
...
The key insight is that context accumulates. Every review comment that leads to an updated `L10N.md` or a corrected terminology entry improves all future runs, not just the file that triggered the review.
