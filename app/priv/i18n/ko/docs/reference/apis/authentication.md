%{
  title: "인증 및 권한",
  summary: "Glossia 가 사용자를 인증하고 API 접근 권한을 부여하는 방법.",
  category: "참조",
  subcategory: "API",
  order: 1
}
...
웹 인터페이스를 통해 로그인할 때, Glossia 는 세션 기반 인증을 사용합니다. GitHub 나 GitLab 과 같은 제 3 자 제공자를 통해 인증하는 [Assent](https://github.com/pow-auth/assent) 라이브러리를 사용합니다. 로그인 성공 시 세션 쿠키가 설정되어 이후 요청에 사용됩니다.

### Bearer 토큰 (OAuth 2.1)

API 액세스 (예: CLI 도구나 기타 도구) 에 Glossia 는 OAuth 2.1 과 인증 코드 흐름, PKCE 를 구현합니다. 클라이언트는 Bearer 토큰을 얻어 이를 포함하며 `Authorization` header:
...
클라이언트들은 호출하여 등록합니다 `POST /oauth/register` 자신의 메타데이터와 함께합니다. 이는 다음을 따릅니다 [RFC 7591](https://datatracker.ietf.org/doc/html/rfc7591).
...
클라이언트는 사용자를 리디렉션합니다 `/oauth/authorize` PKCE 매개변수를 사용하여:
...
**모든 클라이언트에 PKCE 가 필요합니다.** 오직 `S256` 챌린지 방법만 지원됩니다.

### 3\. 토큰 교환

사용자가 승인한 후 클라이언트는 토큰을 위한 인증 코드를 다음에서 교환합니다 `POST /oauth/token`:
...
응답에는 액세스 토큰이 포함되며 선택적으로 리프레시 토큰도 포함됩니다.

### 4\. 토큰 갱신

액세스 토큰이 만료되면 리프레시 토큰을 사용하세요:
...
스코프는 토큰이 수행할 수 있는 동작을 제어합니다. 다음 규칙을 따릅니다 `object:action` 패턴.
...
| `organization:read` | 조직 세부 정보 읽기 (내 조직 목록 확인 포함) |
...
Glossia 는 강제합니다 **두 가지 레이어** REST API 와 MCP 서버를 위한:

1. **범위 확인**: 액세스 토큰에는 필수 `object:action` 범위.
2. **리소스 수준의 정책**: 현재 사용자는 특정 리소스에 대해 다음을 통해 허가되어야 합니다 `Glossia.Policy`.

스코프는 나타내는 *최대* 토큰의 권한. 정책 시스템은 강제하는 *실제* 특정 리소스에 대한 권한.
...
Glossia 는 표준적이고 잘 알려진 URL 에서 메타데이터를 게시하여 클라이언트가 자동으로 엔드포인트를 발견할 수 있도록 합니다.

### OAuth 인증 서버 메타데이터 (RFC 8414)

    GET /.well-known/oauth-authorization-server

발급자, 엔드포인트, 지원되는 범위, 권한 부여 유형, 및 코드 챌린지 방법을 반환합니다.
...
| `POST /oauth/revoke` | 1 분당 30 회 요청 |
| `POST /oauth/introspect` | 1 분당 30 회 요청 |
