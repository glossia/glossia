%{
  ...
  summary: "Cómo Glossia autentica usuarios y autoriza el acceso a la API.",
  category: "Referencia",
  subcategory: "APIs",
  order: 1
}
...
Cuando inicia sesión a través de la interfaz web, Glossia utiliza autenticación basada en sesión. Se autentifica a través de un proveedor de terceros (GitHub o GitLab) mediante el [Assent](https://github.com/pow-auth/assent) librería. Después de un inicio de sesión exitoso, se establece una cookie de sesión y se utiliza para peticiones posteriores.

### Tokens Bearer (OAuth 2.1)

Para el acceso a la API (como desde la CLI u otras herramientas), Glossia implementa OAuth 2.1 con el flujo de código de autorización y PKCE. Los clientes obtienen un token Bearer y lo incluyen en el `Authorization` cabecera:
...
Los ámbitos controlan qué acciones puede realizar un token. Siguen el `object:action` patrón.
...
| `members:read` | Leer miembros de la organización y las invitaciones |
| `members:write` | Gestionar miembros de la organización y las invitaciones |
...
| `glossary:admin` | Gestionar configuración de terminología |

## Modelo de autorización

Glossia exige **dos capas** para la API REST y el servidor MCP:
...
Alcances representan el *máximo* capacidad de un token. El sistema de políticas aplica el *efectivo* permiso para un recurso específico.
...
### Metadatos del servidores de autorización OAuth (RFC 8414)

    GET /.well-known/oauth-authorization-server

Devuelve el emisor, endpoints, scopes soportados, tipos de concesión y métodos de desafío de código.

### Metadatos de recurso protegido (RFC 9728)

    GET /.well-known/oauth-protected-resource

Devuelve el identificador de recurso, servidores de autorización, scopes soportados y métodos de portador.
...
Cuando se aplica la limitación de tasa, el servidor devuelve HTTP 429 (Demasiadas solicitudes).