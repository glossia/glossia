...
Cuando inicias sesión a través de la interfaz web, Glossia utiliza autenticación basada en sesión. Te autentificas a través de un proveedor de terceros (GitHub o GitLab) usando el [Assent](https://github.com/pow-auth/assent) librería. Después de un inicio de sesión exitoso, se establece una cookie de sesión y se utiliza para las solicitudes posteriores.
...
Los clientes se registran por sí mismos llamando `POST /oauth/register` con sus metadatos. Esto sigue [RFC 7591](https://datatracker.ietf.org/doc/html/rfc7591).
...
El cliente redirige al usuario a `/oauth/authorize` con los parámetros PKCE:
...
Glossia impone **dos capas** para la REST API y el servidor MCP:
...
Ámbitos representan el *máxima* capacidad de un token. El sistema de políticas aplica el *efecto* permiso para un recurso específico.
...
Glossia publica metadatos en URLs estándar bien conocidas para que los clientes puedan descubrir los puntos de conexión automáticamente.
...
Devuelve el identificador del recurso, los servidores de autorización, los ámbitos admitidos y los métodos bearer.

## Limitación de tasa

Los puntos finales OAuth tienen límites de tasa por dirección IP:
