...
通过 Web 界面登录时，Glossia 使用基于会话的认证。您通过第三方提供商（GitHub 或 GitLab）进行认证，使用 [Assent](https://github.com/pow-auth/assent) 库。成功登录后，会设置会话 Cookie 并用于后续请求。

### Bearer 令牌（OAuth 2.1）

用于 API 访问（例如来自 CLI 或其他工具），Glossia 实现了 OAuth 2.1 授权码流程和 PKCE。客户端获取 Bearer 令牌并将其包含在 `Authorization` 头部：
...
客户端通过调用注册自身 `POST /oauth/register` 及其元数据。这遵循 [RFC 7591](https://datatracker.ietf.org/doc/html/rfc7591).
...
客户端将用户重定向至 `/oauth/authorize` 包含 PKCE 参数：
...
**PKCE 对于所有客户端都是必需的。** 只有 `S256` 挑战方法被支持。

### 3\. 令牌交换

用户批准后，客户端在将授权代码交换为令牌 `POST /oauth/token`:
...
Glossia 在标准公认 URL 上发布元数据，以便客户端能自动发现端点。

### OAuth 授权服务器元数据 (RFC 8414)

    GET /.well-known/oauth-authorization-server

返回签发者、端点、支持的授权范围、授权类型以及代码挑战方法。
