...
3. Cliquez sur **Jetons de compte**Ensuite **Nouveau jeton**.
4. Donnez au jeton un descriptif **nom** (par exemple, "CI deploy" ou "CLI access").
5. Choisissez les **périmètres** dont le jeton a besoin. N'accordez que les autorisations minimales requises.
...
Les jetons de compte suivent les mêmes [modèle d'autorisation](/docs/reference/apis/authentication) via des jetons OAuth. Les portées du jeton définissent l'ensemble maximal d'actions qu'il peut effectuer, et les politiques de niveau de ressource s'appliquent toujours en fonction des relations de votre compte.
...
La **Jetons de compte** la page liste tous les jetons actifs avec leur nom, leurs portées, leur date de dernière utilisation et leur expiration. Les jetons qui n'ont jamais été utilisés affichent "Jamais" dans la colonne de dernière utilisation.

### Édition des jetons

Cliquez sur le nom d'un jeton pour modifier son **nom** et **description**. Les portées et l'expiration ne peuvent pas être modifiées après la création. Si vous avez besoin de différentes portées, créez un nouveau jeton et révoquez l'ancien.
...
- **Utilisez des jetons à courte durée de vie.** Définir une date d'expiration lorsque c'est possible.
- **Minimiser les portées.** Accorder uniquement les autorisations dont le jeton a réellement besoin.
- **Renouveler régulièrement.** Créer de nouveaux jetons et révoquer les anciens selon un calendrier.
...
- **Utiliser un jeton par intégration.** Ainsi, la révocation d'un jeton ne perturbe pas d'autres flux de travail.
