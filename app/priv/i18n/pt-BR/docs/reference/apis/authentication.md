...
Quando você faz login pela interface web, o Glossia usa autenticação baseada em sessão. Você se autentica por meio de um provedor de terceiros (GitHub ou GitLab) usando o [Assent](https://github.com/pow-auth/assent) biblioteca. Após um login bem-sucedido, um cookie de sessão é definido e usado para solicitações subsequentes.

### Tokens Bearer (OAuth 2.1)

Para acesso via API (como do CLI ou outras ferramentas), o Glossia implementa OAuth 2.1 com o fluxo de código de autorização e PKCE. Os clientes obtêm um token Bearer e o incluem `Authorization` cabeçalho:
...
Clientes se registram ao chamar `POST /oauth/register` com seus metadados. Isso segue [RFC 7591](https://datatracker.ietf.org/doc/html/rfc7591).
...
**O PKCE é obrigatório para todos os clientes.** Apenas o `S256` método de desafio é suportado.
...
A resposta inclui um token de acesso e, opcionalmente, um token de refresco.

### 4\. Renovação de token

Quando um token de acesso expira, use o token de refresco:
...
Os escopos controlam quais ações um token pode executar. Eles seguem o `object:action` padrão.
...
Glossia impõe **duas camadas** para a REST API e o MCP server:

1. **Verificação de escopo**: o token de acesso deve incluir o necessário `object:action` escopo.
2. **Política de nível de recurso**: o usuário atual deve estar autorizado para o recurso específico via `Glossia.Policy`.
...
O Glossia publica metadados em URLs padrão bem conhecidas para que os clientes possam descobrir endpoints automaticamente.
