%{
  ...
  summary: "Como a Glossia autentica os usuários e autoriza o acesso à API.",
  category: "Referência",
  subcategory: "APIs",
  order: 1
}
...
O Glossia suporta dois métodos de autenticação conforme o contexto.

### Sessões de navegador

Ao fazer login pela interface web, o Glossia usa autenticação baseada em sessão. Você se autentica via um provedor de terceiros (GitHub ou GitLab) usando a [Assent](https://github.com/pow-auth/assent) biblioteca. Após um login bem-sucedido, um cookie de sessão é definido e utilizado para requisições subsequentes.

### Tokens Bearer (OAuth 2.1)

Para acesso à API (como da CLI ou de outras ferramentas), o Glossia implementa OAuth 2.1 com o fluxo de código de autorização e PKCE. Os clientes obtêm um token Bearer e o incluem no `Authorization` cabeçalho:
...
**PKCE é obrigatório para todos os clientes.** Somente o `S256` método de desafio é suportado.
...
Quando um token de acesso expirar, use o token de renovação:
...
Os escopos controlam quais ações um token pode executar. Eles seguem a `object:action` padrão.

| Escopo | Descrição |
|-------|-------------|
| `user:read` | Ler informações do perfil do usuário |
| `user:write` | Atualizar perfil do usuário |
| `account:read` | Listar as contas da organização às quais você tem acesso |
...
Glossia garante **duas camadas** para a REST API e o servidor MCP:

1. **Verificação de escopo**: o token de acesso deve incluir o necessário `object:action` escopo.
2. **Política de nível de recurso**: o usuário atual deve ser autorizado para o recurso específico por `Glossia.Policy`.

Escopos representam o *máximo* capacidade de um token. O sistema de políticas impõe a *efetiva* permissão para um recurso específico.
...
| `organization_admin` | Administrador da organização que possui o recurso |
...
A Glossia publica metadados em URLs padrão e bem conhecidas para que os clientes descubram endpoints automaticamente.
