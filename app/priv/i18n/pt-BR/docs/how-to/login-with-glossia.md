...
---
Este guia o orienta a adicionar "Entrar com o Glossia" ao seu aplicativo. Ao final, seus usuários poderão fazer login com sua conta do Glossia e seu aplicativo terá um token de acesso para chamar a API do Glossia em seu nome.

O Glossia usa **OAuth 2.1 com PKCE** (Chave de Prova para Troca de Código). PKCE é obrigatório para todos os clientes, incluindo aplicativos de lado do servidor.
...
1. Faça login no Glossia e acesse o painel da sua conta.
2. Abra a **API** seção na barra lateral e clique **Aplicativos OAuth**.
3. Clique **Nova aplicação**.
4. Preencha a aplicação **nome** e **URL de callback** (também chamado de URI de redirecionamento).
...
### Opção B: Cadastro dinâmico do cliente
...
## 3\. Redirecionar o usuário para a Glossia
...
| `scope` | Nº | Lista separada por espaço de [escopos](/docs/reference/apis/authentication). Padrão: acesso mínimo se omitido |
| `state` | Recomendado | Uma string aleatória para prevenir ataques CSRF. Verifique se ela corresponde quando o usuário retornar |

O usuário verá uma tela de consentimento mostrando o nome do seu aplicativo e os escopos solicitados. Após a aprovação, o Glossia redireciona de volta para sua URL de callback com um código de autorização.

## 4\. Troque o código pelos tokens

Quando o usuário for redirecionado de volta para sua URL de callback, a URL conterá um `code` parâmetro:

    https://myapp.com/auth/callback?code=AUTHORIZATION_CODE&state=RANDOM_STATE_VALUE

Primeiro, verifique que `state` corresponda ao que você enviou no passo 3. Em seguida, troque o código pelos tokens:
...
Armazene ambos os tokens com segurança. O token de acesso é usado para requisições de API. O token de renovação é usado para obter um novo token de acesso quando o atual expirar.
...
Os escopos do token limitam quais endpoints você pode acessar. A autorização no nível de recursos ainda se aplica - por exemplo, um token com `project:read` pode ler apenas os projetos aos quais o usuário tem acesso.

## 6\. Renove o token

Quando o token de acesso expirar, use o token de renovação para obter um novo sem passar o usuário pelo fluxo de consentimento novamente:
...
Seu aplicativo pode descobrir automaticamente os endpoints OAuth do Glossia buscando os metadados do servidor:

```bash
curl https://glossia.ai/.well-known/oauth-authorization-server
```

Isso retorna um documento JSON com o `authorization_endpoint`, `token_endpoint`, `revocation_endpoint`e outros detalhes. O uso da descoberta torna sua integração resistente a alterações nos endpoints.
...
Antes de colocar em produção, verifique se sua implementação segue estas práticas:
...
- Trate a expiração dos tokens com fluidez usando tokens de atualização
- Revogue os tokens quando os usuários se desconectarem ou excluir sua conta