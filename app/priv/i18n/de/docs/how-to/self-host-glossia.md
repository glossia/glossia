...
- Ein Domainname, den Sie auf den Cluster Ingress verweisen können
- Ein OpenID Connect-Provider oder SMTP-Relay zur Authentifizierung (Glossia unterstützt beides)

Das Helm-Chart bündelt Postgres (über [CloudNativePG](https://cloudnative-pg.io/)) und ClickHouse (über den [offiziellen Operator](https://github.com/ClickHouse/clickhouse-operator)) sodass Sie keine externen Datenbanken benötigen. Wenn Sie lieber eigene nutzen möchten, können beide deaktiviert in `values.yaml`,

## Installieren Sie die Operatoren

Installieren Sie die Operatoren, die für die Komponenten passen, die Sie aktivieren möchten. Mindestens:
...
- [cert-manager](https://cert-manager.io/) wenn Sie automatisches TLS benötigen
...
Erstellen Sie einen Kubernetes Secret namens `glossia-app-env` mit mindestens diesen Schlüsseln:
...
Sie können diese direkt bereitstellen, verwenden [Sealed Secrets](https://github.com/bitnami-labs/sealed-secrets), oder verbinde den Chart mit deinem Secrets Manager über den [External Secrets Operator](https://external-secrets.io/) Integration beschrieben in der Chart README.
...
- Hintergrundprozesse für Übersetzungsjobs, die als Kubernetes-Jobs ausgeführt werden, damit sie rollierenden Bereitstellungen überdauern
...
- Melden Sie Probleme oder schlagen Sie Verbesserungsvorschläge unter [GitHub-Repository](https://github.com/glossia/glossia).