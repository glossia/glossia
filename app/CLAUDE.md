...
- **Dev instance scoping**: Each clone/worktree gets a unique numeric suffix (1-999) persisted in `.glossia-dev-instance`. The suffix scopes app ports (`GLOSSIA_SERVER_PORT`, `GLOSSIA_TEST_PORT`) and database names (`GLOSSIA_POSTGRES_DB`, `GLOSSIA_CLICKHOUSE_DB`, etc.) via `mise/utilities/dev_instance_env.sh`, which is sourced automatically by mise. Infrastructure services (PostgreSQL, ClickHouse) share default ports. Override the suffix with `GLOSSIA_DEV_INSTANCE` env var. When using a fresh clone, run `cd app && mix ecto.create && mix ecto.migrate`.
...
CSS follows a strict convention for targeting elements — no BEM, no utility classes, no class-based methodologies.

**Shared components** — When you identify a reusable UI pattern, extract its CSS into `assets/css/components/{id}.css` where `{id}` matches the component's root `id` attribute. Target the root element by its `id` and nested elements with `data-part` attributes using CSS nesting:

```css
/* assets/css/components/model-card.css */
#model-card {
  display: flex;
  gap: var(--space-4);

  & > [data-part="header"] {
    font: var(--weight-medium) var(--text-sm);
...
**Pages** — Target each page with a unique `id` on the root element. Nest child selectors using `data-part`:

```css
#llm-models-page {
  & > [data-part="form-section"] { ... }
  & > [data-part="model-list"] { ... }
}
```

**General rules**:
- Never use BEM (`.block__element--modifier`) or utility classes
- Use modern CSS nesting (`& >`) instead of flat selectors
- Never hardcode values — always reference theme tokens via `var()`
- Never try to reuse CSS beyond the CSS variables defined in the theme
- Before writing new styles, check if a similar layout already exists in the codebase to use as reference
