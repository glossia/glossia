defmodule Glossia.Foundation.Localizations.Core.Workers.LocalizationRequestChangesParser do
  def parse_localization_request(request) do
    Enum.map(request[:modules], fn module ->
      ...
      content_and_context_checksum = calculate_checksum(source_content_checksum_value, source_context, source_content_checksum_algorithm)
      ...
      target = if content_and_context_checksum == source_cache_checksum_value && source_content_checksum_algorithm == source_cache_checksum_algorithm do
        []
      else
        Enum.flat_map(target_localizables, fn target_localizable ->
          ...
          _target_content_checksum_algorithm = target_localizable[:checksum][:content][:algorithm]
