defmodule Glossia.Localizations.Utilities.Localizer do
  ...
  def localize(platform_module, id_in_platform, version, content_changes) do
    updates =
      content_changes
      |> Enum.map(fn module ->
  ...
  def localize_module(platform_module, id_in_platform, module, version) do
    {:ok, source_content} =
      platform_module.get_content(
        id_in_platform,
        module[:source][:id],
        version
      )
